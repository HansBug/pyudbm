...
Step 1 生成出来的 `content.md` 绝不能默认当作正确稿。它只是基于文本提取和/或 OCR 生成的临时中间产物，可能存在漏字、错字、公式损坏、结构错乱、图表引用错位等问题。最终是否验收，标准是成稿后的 `content.md` 在视觉上是否与 PDF 本身完全一致，而不是它是否只是比抽取稿“更干净”。
...
- 这个 Markdown 输出只能当作后续人工精修的草稿，不能当作已经可信的 PDF 转写结果
- `image` 模式会把每一页渲染成图像，供视觉校对
- 不要默认同一个页面渲染器对所有页面版式都可靠；遇到横页、旋转页，或者某一页几乎整页都是超宽表格/图片时，不同渲染器可能会把方向弄错，或者在右侧 / 底部提前截断内容
- 不要把页面图像直接导出到 `papers/` 下的论文子目录里；逐页图像通常很大，应当导出到临时路径，例如 `/tmp/...`
- 每次都要显式指定输出路径，不要假定存在 batch 模式或与仓库路径绑定的默认行为

关于横页或旋转页的额外提醒：

- 在第一轮页面图像巡检时，要有意识地检查哪些页的长宽比或阅读方向与前后页面不同
- 如果某一页在 PDF 里明显是横向表格/图片页，但导出的页面图像仍然看起来是竖页、侧着读，或者最右侧 / 最下方内容像是先天缺失，这一页的页面图像就不能再当作可信依据
- 一个典型症状是：你已经反复放大裁切框，但仍然拿不到 PDF 里明明存在的列、caption 或页面区域
- 出现这种情况时，不要继续在这张坏页图上死裁；应当立刻换第二个 PDF 渲染器重新导出该页进行核对。例如先用 `tools.papers_to_content -ot image`，如果怀疑有问题，就再用 `pdftoppm` 之类的渲染器重出该页
- 第二份页面图像出来后，要重新核对该页的方向、完整边界和内容覆盖范围，再基于新的页面图像制作最终图表资源
- 最终图表资源应当保证图或表本体是正向可读的，但不要把只属于页面本身的侧边栏、旋转页眉、页码等页面家具误带进图表资源里
...
- 建清单时还要顺手检查是否存在横页 / 旋转页渲染异常；如果某一页方向看起来不对，或者某个超宽表格 / 图片像是天生就少了一块，就先换渲染器重出这一页，再继续裁图
...
- 如果你已经明显放宽裁切框，但缺边问题仍然存在，要优先怀疑“源页面图像本身就是坏的”，而不是继续微调裁切框；这时应换渲染器重出该页，并基于新页图重做资源
...
- 用来裁图的那张页面图像本身就不可信，例如横页被错误地按竖页渲染，或者超宽表格在裁切前就已经被页面渲染器截掉了一部分
