...
可获取性说明：
本目录里的 `paper.pdf` 现在是一份 14 页的完整扫描版章节 PDF，`content.md` 则是基于该扫描件逐页核对后的人工精修 Markdown 转写。

实践上的注意点：
这仍然是一份老会议文集的扫描件，而不是原生数字 PDF，所以把 automata / region 图单独裁成 `content_assets/` 并和正文一起保留，对核对记号和结构仍然很重要。
