...
如果说 `ta_tools` 回答的是“怎样对 zone 做符号算法”，那这篇更偏向回答“clock 和 timed automata 这个对象本身到底是什么”。
...
如果你想先看历史源头，可以放在 `ta_tools` 之前读；如果你已经先读了 `ta_tools`，那它适合作为回填 formal roots 的一篇。

可获取性说明：
本目录里的 `paper.pdf` 只是 Springer 预览页，不是完整章节全文。
