...
在二维几何上，刚才这个 zone 的形状其实很直观。下面这张图故意保持语言中性，只画
:math:`x` / :math:`y` 坐标轴、边界线和填充区域；它表达的正是同一个
...
在数学层面上，把 :math:`D_{ij}` 讲成二元组 :math:`(\triangleleft_{ij}, c_{ij})` 很方便。
...
下面这张 before / after 图里，左边是输入区域 :math:`Z`，右边是加上
:math:`y \geq 2`
之后的结果；右边虚线保留了原区域轮廓，方便直接看出求交到底删掉了哪一部分：
...
下面这张图把这个“向未来扩张”的效果直接画出来了：左边是原区域，右边是 ``up`` 之后在同一观察窗口里的几何切片。
你可以直接看到绝对上界消失后，区域如何沿着仍然保留的差值约束向右上方延伸：
...
对应的几何直觉见下图：左边还是输入区域，右边是 ``down`` 之后在同一观察窗口中的样子。
和 ``up`` 相反，这次区域是在向更早、通常也更靠左下方的赋值扩展开来：
...
几何上，reset 最容易被看成“把区域压到某个固定的时钟值上”。下面这张图里，右边那条绿色竖线就是
:math:`x := 2`
之后留下来的结果；虚线仍然表示原区域边界：
...
这一点在几何上会表现成“沿某个方向把约束放开”。下面这张图里，右边就是 ``free`` 一个时钟之后的区域切片；
它保留了剩余时钟关系，但对被释放的方向明显更宽松：
