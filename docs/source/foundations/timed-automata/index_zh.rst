...
* :math:`L` 是有限个 location 的集合
* :math:`\ell_0 \in L` 是初始 location
* :math:`C` 是有限个 clocks 的集合
* :math:`\Sigma` 是动作标签集合
* :math:`E \subseteq L \times \Sigma \times G(C) \times 2^C \times L` 是边关系
* :math:`\mathrm{Inv}` 给每个 location 指定一个 invariant
...
这不是随便写出来的一套符号。后面 zone / DBM 能成立，关键就在于 guard 和 invariant
都是由这类简单时钟约束做合取得到的，而不是任意实数公式 [AD90_ZH]_ [BY04_ZH]_。

回到上面的运行中的小例子，这个 tuple 可以具体展开成：
...
* :math:`\mathrm{Inv}` 则给每个 location 一个不变量：
...
UPPAAL 网络语义里对动作方向做的补充标记：这里的 automaton 会发送 ``send`` 和 ``timeout``，
并接收 ``ack``。但就上面的数学 tuple 而言，先把这些动作名记成
...
* :math:`v` 是一个 valuation，也就是“当前每个时钟各自取多少值”
...
\ **valuation :math:`v` 会给每个时钟分配一个非负实数值。**

在这个例子里，因为只有一个时钟 ``x``，所以一个 valuation 基本上就是在说：
...
因此一个具体 timed state 会写成 :math:`(\ell, v)`，其中：

* :math:`\ell` 是当前 location
* :math:`v` 是当前所有时钟的取值
...
* :math:`(v + d)` 表示“在 valuation :math:`v` 的基础上，让所有时钟一起增加 :math:`d`”
* :math:`(v + d)(x)` 表示“增加完之后，时钟 :math:`x` 的值是多少”
* :math:`R` 是要被 reset 的时钟集合
* :math:`v[R := 0]` 表示“从 :math:`v` 出发，把 :math:`R` 里的时钟全部改成 `0` 后得到的新 valuation”
* :math:`v[R := 0](x)` 表示 reset 之后时钟 :math:`x` 的值
...
* 如果当前 :math:`v(x) = 4`，并且发生某个把 ``x`` reset 的动作，那么对 :math:`R = \{x\}` 有
  :math:`v[R := 0](x) = 0`

回到前面的例子里，``(WaitAck, x = 1)`` 和 ``(WaitAck, x = 4.9)`` 虽然都在同一个 location，
...
* :math:`\models` 表示“满足”；例如 :math:`v \models x \le 5` 意味着在 valuation :math:`v` 下约束 :math:`x \le 5` 为真
* :math:`\mathrm{Inv}(\ell)` 表示 location :math:`\ell` 的 invariant

这里最关键的是对 :math:`\delta` 的全称条件。它表示 invariant 必须在整段等待区间里都成立，
...
* :math:`g` 是 guard
* :math:`R` 是 reset 集合
* :math:`\ell'` 是目标 location

这就把几种角色分清楚了：

* **guard** 决定这条边什么时候可用
* **reset 集合** 决定哪些时钟会跳回 `0`
* **目标不变量** 决定跳过去之后能不能合法停在目标位置

下面把前面的例子改画成“具体 timed states 的演化”：

.. graphviz:: state_evolution.dot

这张图想强调的语义点很简单但很重要：系统可以从 ``x = 0`` 一直拖到 ``x = 5``，但不能再继续等。
一旦到达这个边界，接下来就必须发生离散跳转；否则运行会被 invariant 卡死。
...
其中每个 :math:`\ell_i` 是一个进程当前所在的 location，:math:`v` 是共享的时钟赋值。

直觉上可以这样理解：

* 时间流逝是全局的，所以所有时钟一起增长
* 离散步可以是某一个 automaton 自己走，也可以是多个 automata 通过 channel 同步一起走
* verifier 并不会一个 valuation 一个 valuation 地枚举这个无限状态空间，而是后面会用符号状态来打包处理

这一页刻意先停留在单个自动机上，因为 clocks、guards、resets、invariants 的基本语义，
在这个最小场景里就已经完整出现了。
...
* \ **location 不是完整状态。** valuation 也是状态的一部分。
* \ **invariant 不是普通 guard。** guard 管的是“能不能走某条边”；invariant 管的是“还能不能继续待在这里”。
* \ **到达边界往往会逼出一个离散选择。** 如果再等已经不合法了，那下一步不是走边，就是 deadlock。

这和 ``pyudbm`` 有什么关系
---------------------------

这个仓库现在还没有把完整的 UPPAAL 语言和网络语义都恢复出来，但时间自动机这套视角已经足够解释：
为什么 Python API 必须是以 clock 为中心，而不是只暴露匿名矩阵操作。

可以直接把对应关系看成：

* ``Context`` 固定时钟集合 :math:`C`
* ``Clock`` 用来构造 guard / invariant / clock-difference constraints
* ``Federation`` 表示满足这些约束的一批 valuations

所以 `pyudbm` 暴露的并不是“某种偶然的数据结构”，而是在暴露 guards、invariants 和 reachable timed states
背后的符号约束层。
...
* 时间自动机 = 有限控制图 + clocks
* 一个具体状态是 ``(location, valuation)``
* 时间流逝和离散跳转是两类不同的迁移
* invariant 决定一个 location 最多还能停多久
* 这些时钟约束正是后面 zones 和 DBMs 的语义来源

下一步
------

下一篇最自然的内容是 ``queries-and-properties/``：模型长什么样讲清楚以后，接下来就是用户通常会问它什么性质。
