...
官方文档对验证器(verifier) 的描述很直接：它会通过对符号状态空间(symbolic state-space)的即时探索
来检查安全性(safety) 和活性(liveness) 一类性质 [UPP_VER_ZH]_。
...
UPPAAL 经典的符号查询(symbolic query)语言，本质上就是围绕这些问题组织起来的
...
UPPAAL 查询是由\ **状态谓词(state predicate)**\ 组合出来的。

状态谓词就是“在某个状态上取真或取假的布尔条件”。按照官方文档，它可以引用
位置、变量、逻辑组合以及时钟约束 [UPP_QSYN_ZH]_ [UPP_QSEM_ZH]_。

典型例子包括：

* ``Controller.WaitAck``：进程 ``Controller`` 当前位于位置 ``WaitAck``
* ``x <= 5``：当前状态满足这个时钟约束
...
这一页主要聚焦前四类里最常见的三类，加上 ``p --> q`` 这种进展性质(progress property)，
因为它们已经覆盖了大部分入门阶段的问题。

最小语义骨架
~~~~~~~~~~~~

UPPAAL 官方对查询的语义，是放在一个带时间迁移系统(timed transition system) 上来写的：
...
也就是说，可达性(reachability)语义关注的是“有没有某条执行能把系统带到某处”，而
...
.. math::
   ...
   \iff
   \exists \text{ 一条极大路径，其上每个状态都满足 } p

逐条把它们翻成人话，可以这样理解：

* :math:`E\langle\rangle p`：
  只要存在\ **一条**\ 执行，能把系统带到某个满足 :math:`p` 的状态，这个查询就为真。
...
* :math:`A\langle\rangle p`：
  不只是说 :math:`p` 可达，而是说每一条极大路径迟早都会进入某个满足 :math:`p` 的状态。
  如果存在哪怕一条执行能永远拖着不进入 :math:`p`，那它就是假的。
* :math:`E[]\, p`：
  存在一条极大路径，可以一直待在满足 :math:`p` 的状态里。
...
这几条写法对教学做了轻度简化，但和官方给出的伪形式语义(pseudo-formal semantics) 是一致方向的
[UPP_QSEM_ZH]_。
...
所以当你想找“一条见证轨迹(witness trace)”时，``E<>`` 往往是最自然的起点。
...
它问的不是“是否存在某条执行到达 ``Acked``”，而是“是否每一条极大执行(maximal execution) 最终都会到达 ``Acked``”。
...
它问的是：是否存在一条极大执行，能一直待在 ``Acked`` 之外。
...
会成为 UPPAAL 教程和示例里最常见的健全性检查(sanity check)之一 [BDL04_QP_ZH]_。
...
查询的价值不只是“逻辑上可判定”，还在于工具通常能告诉你\ **为什么**\ 它真或假。

UPPAAL 的符号验证器(symbolic verifier) 可以生成见证轨迹(witness trace) 或反例轨迹(counterexample trace)，这些轨迹还能在
符号模拟器(symbolic simulator)里继续检查 [UPP_QSEM_ZH]_ [UPP_VER_ZH]_。

实践上大致可以这样理解：

* ``E<> p`` 往往会附带一条到达 ``p`` 的见证路径
* ``A[] p`` 往往会附带一条到达 ``not p`` 的反例
* ``A<> p`` 和 ``p --> q`` 的失败原因，很多时候不是“显式坏状态”，而是“某个无限拖延的循环”

这也是为什么查询的措辞非常重要：两个用自然语言说起来差不多的需求，可能会对应完全不同的诊断轨迹。

超出 yes/no 之后
~~~~~~~~~~~~~~~~

当前 UPPAAL 的符号查询语法还包括 ``sup``、``inf``、``bounds`` 这类定量查询形式，
可以对时钟或整数表达式提问 [UPP_QSYN_ZH]_。

这一页先不展开它们。当前最重要的事情更基础：

* 先分清你问的是存在性(existence)、不变性(invariance)、最终性(eventuality)还是进展(progress)
* 再选与之匹配的查询形式
...
`pyudbm` 现在还没有暴露完整的 UPPAAL 查询语言，但查询的视角已经足够解释：
为什么时钟和时钟约束根本不是“附属细节”。

因为验证器检查的是建基于符号状态之上的性质，所以：

* 查询的真实语义最终落在“满足某些约束的一批赋值”上，而不是单个赋值上
* 高层谓词如 ``x <= 5`` 并不是装饰语法，而是验证真正谈论的对象
* 后面符号状态、区域、差分约束矩阵、联邦这些页面，讲的正是让这些查询变得可做的底层表示
...
* ``A<> p`` 比可达性强，它问的是每条路径最终是否都必须到达 ``p``
* ``deadlock`` 在 UPPAAL 里是一个一等状态谓词
* ``p --> q`` 是进展风格性质，不只是另一种可达性
