...
下面看一个只有一个时钟 ``x`` 的极小自动机。它足够小，小到可以把每一步搜索都算出来；
...
也就是说，验证器并不是在追踪某个单独时刻 ``x = 2`` 或 ``x = 3.4``。
它在追踪的是：

* ``L_0`` 中的整段可行时间窗口 :math:`0 \le x \le 5`
* 穿过第一条边后在 ``L_1`` 中的整段窗口 :math:`0 \le x \le 3`
* 再穿过第二条边后在 ``Goal`` 中的整段窗口 :math:`0 \le x \le 1`
...
从搜索循环的角度看，同一件事还可以写成一张更工程化的 ``WAIT`` / ``PASSED`` 演化表：
...
.. list-table::
   ...
   * - 0
     - :math:`(L_0, Z_0)`
     - 加入 :math:`PASSED(L_0)`
     - 生成 :math:`(L_1, Z_1)` 并放回 ``WAIT``
   * - 1
     - :math:`(L_1, Z_1)`
     - 加入 :math:`PASSED(L_1)`
     - 生成 :math:`(Goal, Z_2)` 并放回 ``WAIT``
   * - 2
     - :math:`(Goal, Z_2)`
     - 命中目标
     - 返回“可达”，并可继续回溯诊断轨迹
...
它们和 :meth:`Federation.contains`、``<=`` 这类覆盖判断 API 放在一起看，才更容易理解：
