...
- 中文页里的 graphviz 图也应做中文化处理；图中的说明性文字，如标题、cluster 标签、节点说明、边标签，默认都应使用中文
- 只有代码字面量、页面 slug、官方名称、模型里刻意保留的动作名 / 位置名 / 缩写，才应继续保留英文
- 对几何型的 zone / DBM 图，优先使用仓库内可生成的 `.plot.py -> .plot.py.svg` 方式，而不是手工截图
- 如果图本身主要承担几何直觉展示，优先做语言中性设计；把主要解释留给页面正文和 caption，这样中英文通常可以共用同一份 SVG
- 对 `up`、`down`、`reset`、`free`、`constrain` 这类操作图，优先使用 before/after 双侧对比布局，让读者直接看到区域如何变化
...
- 不要在一页里开太多同级别的高级 section；如果这些内容其实都在解释同一主线，就应当被归并到更少的主 section 下面
- 先判断这一页真正的主要脉络，再设计有限数量的主 section，让目录视角下的逻辑一眼可见
- 更细的定义、实现细节、例子、性质区分、源码解读，通常更适合收进对应主 section 的下级小节，而不是各自独立升成新的高级 section
- 如果读者扫一眼目录会感觉“这些标题都像在讲同一件事，只是切得太碎”，那就应该继续归并
...
- 如果用 LaTeX 展示 DBM 矩阵，矩阵内部必须显式带上行列时钟坐标头，例如 `x_0`、`x`、`y`；不要只给一个无坐标的裸矩阵
- DBM 矩阵里的单元格展示也应与正文记号保持一致；如果正文采用 `(\triangleleft, c)` 这种二元对写法，矩阵里也应直接写成同款形式
...
- 除代码字面量、路径、页面 slug、外部文档官方标题或专有名词缩写外，中文句子里不应继续大段混入英文术语
- 如果中文术语已经足够自然，就不要再写成“中文 + 英文名词连续并列”的句子
- 写完一页后，应专门回看一次，清掉那些“第一次已经括注，后面却又反复直接写英文”的残留
- 同样也要回看本页附带的 graphviz 图，避免正文已经中文化、图里的说明文字却仍然停留在英文
...
- 用英文术语替代本来已经很稳定的中文表达，例如在正文里反复写 `query`、`guard`、`location`、`valuation`
...
* [x] `cdd/` 主题页完成中英双语骨架
* [x] 搜索 / extrapolation / storage 主题页完成中英双语骨架
* [x] 每页都明确说明它在回答什么工程问题
* [x] 每页都至少给出 1 个“为什么基础表示不够”的具体例子
* [x] 每页都至少给出 1 个算法流程图、状态流图或结构对比图
* [x] 每页公式都保持“少而关键”，避免直接抄论文大段定义
* [x] 每页都明确和前面基础主题的衔接关系
