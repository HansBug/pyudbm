...
* [ ] 确认 `tutorials/` 与 `foundations/` 的职责边界
* [ ] 确认 `foundations/` 根目录与主题子目录的目录规则
* [ ] 确认中英成对页面规则
* [ ] 确认“主题子目录只放 `index.rst` / `index_zh.rst` + 资源文件”的约束
* [ ] 确认入口页要承担阅读分流功能，而不是普通目录页
* [ ] 确认每篇页面都必须以例子驱动，而不是定义驱动
* [ ] 确认公式统一使用 LaTeX，并在 `rst` 中采用 `:math:` 与 `.. math::`
* [ ] 确认不要使用 `$...$` / `$$...$$` 这类 Markdown 数学写法

### Phase 1：根入口与阅读分流页

* [ ] 起草 `docs/source/foundations/index.rst`
* [ ] 起草 `docs/source/foundations/index_zh.rst`
* [ ] 在入口页写清楚 `tutorials/`、`foundations/`、`papers/` 三者的分工
* [ ] 在入口页加入“如果你不熟悉 X，应先读 Y”的阅读分流
* [ ] 在入口页加入完整主题目录
* [ ] 在入口页说明这些页面和未来 Python API 重建路线的关系
* [ ] 检查中英入口页的结构是否对齐
