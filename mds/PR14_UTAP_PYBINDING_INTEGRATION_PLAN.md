...
* [ ] 新增 `pyudbm.binding.utap` Python 包装层。
* [ ] `load_xml` / `load_xta` 返回 `ModelDocument`。
* [ ] `ModelDocument` 提供 `templates`、`processes`、`queries`、`options`、`features`。
* [ ] 模型结构对象提供基础只读字段：`name`、`id` 或稳定索引、`position`、`declaration`、`parameter`、location / edge 关系字段。
* [ ] 明确父子对象生命周期关系。

阶段测试要求：

* [ ] 官方样本集中的 `XML_MODEL_FILE` 全量参数化通过。
* [ ] 官方样本集中的 `TEXTUAL_MODEL_FILE` 全量参数化通过。
* [ ] 对每类结构对象的数量做直接比对。
* [ ] 对每类结构对象的名称列表做直接比对。
* [ ] 对每类结构对象的位置字段做直接比对。
* [ ] 对每类结构对象的布尔/枚举字段做直接比对。
* [ ] 不接受仅用 `assert len(...) > 0` 替代具体字段断言。
...
* [ ] `expression_t` 包装至少提供 `text`、`kind`、`position`、`type`、`size`、`children`、`is_empty`。
* [ ] `type_t` 包装至少提供关键只读判定和文本能力。
* [ ] `symbol_t` 包装至少提供名字、种类、关联类型等可读信息。
* [ ] `position_t` 与 error/warning 对象提供稳定可读字段。
* [ ] `ModelDocument` 暴露 errors / warnings 读路径。
* [ ] 建立“已映射字段清单”和“暂未映射字段清单”。

阶段测试要求：

* [ ] 代表性样本驱动的字段覆盖参数化测试落地。
* [ ] 每个已承诺字段至少有一个样本直接断言。
* [ ] 对 errors 的 `line` / `column` / `message` / `context` 做结构化直接断言。
* [ ] 对 warnings 的 `line` / `column` / `message` / `context` 做结构化直接断言。
* [ ] 对 `position` 对象做结构化直接断言。
* [ ] 默认不允许仅用 substring 判断字段正确性。
