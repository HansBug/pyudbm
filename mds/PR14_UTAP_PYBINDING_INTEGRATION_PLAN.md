...
* [x] 实现 `load_query(path, document, *, builder="auto")`。
* [x] 实现 `loads_query(text, document, *, builder="auto")`。
* [x] 实现 `parse_query(text, document, *, builder="auto")`。
* [x] `ModelDocument` 增加 `load_query(path, *, builder="auto")`。
* [x] `ModelDocument` 增加 `loads_query(text, *, builder="auto")`。
* [x] `builder="auto"` 至少覆盖 `PropertyBuilder` 与 `TigaPropertyBuilder`。
* [x] query 结果对象暴露 `text`、`quantifier`、`options`、`expression`、`is_smc`、`declaration`、`expectation`。

阶段测试要求：

* [x] 官方样本集中的 `QUERY_PROPERTY_FILE` 全量参数化通过。
* [x] 对每个 `.q` 条目先加载 `context_path`。
* [x] 对每个 `.q` 条目再解析 query 文件。
* [x] 对每个 `.q` 条目直接断言返回 query 数量。
* [x] 对每个 `.q` 条目直接断言 quantifier / builder 路径 / 关键字段。
* [x] 对普通 query 与 TIGA query 分别保留代表性样本组。
* [x] 对 `builder="auto"`、显式 builder、错误 builder 都补失败路径测试。

已完成进度回填：

当前已经落地的内容：

* [x] `_utap` 已新增 `load_query` / `loads_query` / `parse_query` native 入口。
* [x] `pyudbm.binding.utap` 已新增 `ParsedQuery` / `ParsedQueryExpectation` dataclass。
* [x] `pyudbm.binding.utap` 已新增模块级 `load_query` / `loads_query` / `parse_query`。
* [x] `ModelDocument` 已新增 `load_query` / `loads_query` / `parse_query` 方法。
* [x] `pyudbm.binding.__init__` 已重新导出 query 工作流 API 与 `ParsedQuery` 类型。
* [x] `builder="auto"` 已实现为“先 `PropertyBuilder`，失败后清理临时诊断再回退 `TigaPropertyBuilder`”。
* [x] query 结果对象当前已暴露 `line`、`no`、`builder`、`text`、`quantifier`、`options`、`expression`、`is_smc`、`declaration`、`result_type`、`expectation`。

已经新增的对应文件：

* [x] `test/binding/test_utap_query.py`
* [x] `test/binding/utap_phase4_data.py`
* [x] `test/testfile/official/query_expectations.json`

本阶段已经实际完成的验证：

* [x] 官方保留样本中的 `47` 个 `QUERY_PROPERTY_FILE` 已按 `catalog.json` 中的 `context_path` 全量参数化通过。
* [x] 每个 `.q` 条目的 query 数量、builder 路径、quantifier、text、expression kind/size、`is_smc`、`result_type` 都已做精确比对。
* [x] 普通 property、TIGA-only、mixed control+SMC 三类代表性样本都已补 public API 测试。
* [x] `builder="auto"`、显式 `property`、显式 `tiga`、错误 builder 值都已补成功/失败路径测试。
* [x] `_utap` native query 入口也已补对应的 smoke 与失败路径测试。

本阶段实现与原计划的偏差：

* [x] `ModelDocument` 除 `load_query` / `loads_query` 外，额外提供了 `parse_query` 方法。
原因：当前实现中 `parse_query` 与 `loads_query` 语义等价，补成方法接口能让 public surface 更完整且不增加语义复杂度。
* [x] `load_query(path, ...)` 当前优先走真实 `FILE*` 解析路径；`loads_query` / `parse_query` 则通过受控临时文件桥接到同一上游 `parse(FILE*)` 逻辑。
原因：少数官方 `.q` 在 `PropertyBuilder::parse(const char*)` 上会失败，但在 `parse(FILE*)` 上稳定成功；当前阶段优先对齐已验证的官方工作流。
* [x] 少数 `probaMin*` 顶层 query 的 `Expression.text` / `ParsedQuery.text` 当前会回退为 quantifier 名称，而不是完整 pretty string。
原因：上游 `expression_t::str()` 在这些顶层 SMC kind 上会抛 `Unexpected index`；当前阶段优先保证 query 工作流稳定可用，并保留完整 quantifier / expression kind / child structure 读路径。
* [x] query expression 的 `position.start/end` 当前不进入阶段测试 golden value。
原因：同一 `Document` 上重复 query parse 时，UTAP 内部 position index 会继续累积，导致绝对偏移不适合作为稳定断言；当前阶段测试改为直接断言 `line` / `column`。

已实际跑过的验证命令与结果摘要：

* [x] `make build`
* [x] `python -m pytest test/binding/test_utap_native.py test/binding/test_utap_query.py -m unittest -q`
* [x] `python -m pytest test/binding -m unittest -q`

对应结果摘要：

* [x] Phase 4 专项 query 测试：`64 passed`
* [x] `test/binding` 全量 unittest（含 Phase 4 新增测试）：`336 passed`

当前状态：
Phase 4 已完成；official `.q` 文件工作流、自动 builder 路径与 public/native 两层测试都已落地。
